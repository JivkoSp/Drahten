...
namespace TopicArticleService.Infrastructure.Logging
{
    ...
    internal sealed class LoggingCommandHandlerDecorator<TCommand> : ICommandHandler<TCommand> where TCommand : class, ICommand
    {
        ...
        public async Task HandleAsync(TCommand command)
        {
            ...
            try
            {
                _logger.LogInformation($"Started processing {commandTypeName} command.");

                await _commandHandler.HandleAsync(command);
                
                _logger.LogInformation($"Finished processing {commandTypeName} command.");
            }
            catch
            {
                _logger.LogError($"Failed to process {commandTypeName} command.");
