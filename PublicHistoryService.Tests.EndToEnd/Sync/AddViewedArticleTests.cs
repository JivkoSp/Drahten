...
namespace PublicHistoryService.Tests.EndToEnd.Sync
{
    public class AddViewedArticleTests : BaseSyncIntegrationTest
    {
        ...
        private async Task<AddViewedArticleCommand> PrepareAddViewedArticleCommandAsync()
        {
            ...
            var addViewedArticleCommand = new AddViewedArticleCommand(ArticleId: Guid.NewGuid(), UserId: userId, DateTime: DateTimeOffset.Now);
        ...
        public async Task Add_Viewed_Article_Endpoint_Should_Return_Http_Status_Code_Created()
        {
            ...
            var response = await Post(command,
                $"/publichistory-service/users/{command.UserId}/viewed-articles/{command.ArticleId}");
            ...
            response.Headers.Location.ToString()
                .ShouldBe($"/publichistory-service/users/{command.UserId}/viewed-articles/{command.ArticleId}");
        }
        ...
        public async Task Add_Viewed_Article_Endpoint_Should_Add_ViewedArticle_To_The_Database()
        {
            ...
            //ACT
            await Post(command, $"/publichistory-service/users/{command.UserId}/viewed-articles/{command.ArticleId}");

            var viewedArticlesResponse = await Get($"/publichistory-service/users/{command.UserId}/viewed-articles/");
