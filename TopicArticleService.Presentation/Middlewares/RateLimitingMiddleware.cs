        ...
        public async Task InvokeAsync(HttpContext context, RequestDelegate next)
        {
            ...
            try
            {
                ...
                if (!lease.IsAcquired)
                {
                    ...
                    _logger.LogWarning($"TopicArticleService --> Request from IP Address: {clientIP} rejected due to rate limit exceeded!");
                ...
                _logger.LogInformation($"TopicArticleService --> Token used by IP {clientIP}");

                await next(context);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"TopicArticleService --> Error processing request from IP: {clientIP}");
